...
| 206 | Completar modularización frontend (continúa #204) | Hecho: `public/js/lib/purchase-totals.js` (math pura dual-mode) + wiring en `purchase-edit.js` + 10 tests node. Pendiente: cablear `shopping-list.js` (shape distinto vía adapter), extraer cliente API compartido y render helpers de `app.js`/`personal-budget.js`. | Media | Media | ⬜ |
