...
| 179 | Divisa base de Presupuesto Personal (congruencia con inventario) | Inventario tiene `currency` configurable; Presupuesto Personal/Cuotas no tiene divisa base global — cada cuota elige su propia divisa (item #178). Si el usuario quiere reportes/KPIs consolidados en una sola divisa, falta un `base_currency` en `personal_budget_settings` + conversión de transacciones/flujos a esa base. Decisión consciente de no resolverlo ahora (alcance: solo cuotas). | Media | Media | ⬜ |
